...
-- Find out how many books written before 1980 were by authors not from the US.
	...
	SELECT * FROM authors JOIN books ON authors.id = books.author_id WHERE publication_date <1980 AND NOT authors.nationality = 'United States of America';

-- For these last two, you should not need a JOIN.
-- Find all authors whose names start with 'J'.

	SELECT * FROM authors JOIN books ON authors.id = books.author_id WHERE name LIKE '%J%'

-- Find all books whose titles contain 'the'.

	SELECT * FROM authors JOIN books ON authors.id = books.author_id WHERE title LIKE '% the %'
